...
module.exports.run = async ({ event, api, args }) => {
  ...
  if (args[0] == "list") {
  ...
  else api.sendMessage("» ⚠️Box has been approved by admin.\Use help to see more commands☑", idBox, (error, info) => {
    if (error) return api.sendMessage("An error has occurred, making sure that the ID you entered is valid and the bot is in the box! ", threadID, messageID);
      ...
      fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
      fs.writeFileSync(pendingPath, JSON.stringify(pending, null, 2));
      api.sendMessage(`» ✅Successful Box/Thread Approval:\${idBox}\Enjoy Using The Bot☑`, threadID, messageID);
    }
  });
