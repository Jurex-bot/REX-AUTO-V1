    ...
    (async () => {
      ...
      while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
        ...
        try {
          ...
          await api.sendMessage({ body: `𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\n🤖Kyle敦. ဗီူ`, attachment: fs.createReadStream(destPath) }, event.threadID);
  ...
  if (event.body !== null) {
    ...
    if (youtubeLinkPattern.test(videoUrl)) {
      try {
        ...
        file.on('finish', () => {
          file.close(() => {
            api.sendMessage({body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 Youtube\n\n🤖Kyle敦. ဗီူ", attachment: fs.createReadStream(filePath) }, event.threadID, () => fs.unlinkSync(filePath));
          });
        });
      } catch (error) {
