...
module.exports.config = {
  ...
  hasPrefix: true,
  aliases: ['temp'],
  description: 'Manage temporary email using 1 secmail API',
  usage: 'tempmail <generate|inbox|forward> [email]',
  credits: 'User',
  cooldown: 3,
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  if (!command) {
    ...
    try {
      ...
      if (!email) {
        api.sendMessage('No temporary email found. Please generate one first.', event.threadID, event.messageID);
      } else {
    ...
    try {
      ...
      console.error(error);
      api.sendMessage('An error occurred while generating the email.', event.threadID, event.messageID);
    }
    ...
    try {
      ...
      if (messages.length === 0) {
        ...
        api.sendMessage(inboxMessage, event.threadID, event.messageID);
      }
    } catch (error) {
      console.error(error);
      api.sendMessage('An error occurred while fetching the inbox.', event.threadID, event.messageID);
    }
    ...
    try {
      ...
      api.sendMessage(`📧 Forwarding address set to: ${argument}`, event.threadID, event.messageID);
    } catch (error) {
      console.error(error);
      api.sendMessage('An error occurred while setting the forward address.', event.threadID, event.messageID);
    }
  } else {
    api.sendMessage('Invalid command. Use "generate" to create a new temporary email, "inbox" to check the inbox, or "forward [email]" to set an email forwarding address.', event.threadID, event.messageID);
  }
};
